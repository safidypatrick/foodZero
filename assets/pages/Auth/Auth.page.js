...
import { userSetToken } from '@/redux/Auth/auth.action';
...
const Auth = ({ userSetToken }) => {
    ...
    const [userLogin, { fusion }] = useMutation('/auth/login');

    const onSubmit = ({ username, password }) => {
        userLogin({ username, password}).then(response => {
            if (response) {
                const { token, user } = response.data;
                userSetToken(token, user, response);
                const redirect_if = ['/login', '/'];
                if (redirect_if.includes(history.location.pathname)) {
                    if (user?.roles.includes('ROLE_GERANT')) {
                        navigate('/');
                    } else if (user?.roles.includes('ROLE_TITULAIRE')) {
                        navigate('/dashboard')
                    }   
                }
            }
            ...
            <div className="lg:w-1/2 xl:max-w-screen-sm">
                <div className="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
                    <div className="cursor-pointer flex items-center">
                        ...
                        <NavLink to="/" className="text-2xl text-indigo-800 tracking-wide ml-2 font-bold">FoodZero</NavLink>
                    </div>
                </div>

                <div className="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
                    <h2 className="text-center text-4xl text-indigo-900 font-display font-semibold lg:text-left xl:text-5xl
                    xl:text-bold">Se connecter</h2>
                    <div className="mt-12">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div>
                                <div className="text-sm font-bold text-gray-700 tracking-wide">Votre pseudo</div>
                                <input
...
const mapDispatchToProps = (dispatch) => {
    return {
        userSetToken: (token, user, response) => {dispatch(userSetToken(token, user, response))}
    }
}
